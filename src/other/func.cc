...
void read_data(char *filename, Args args, int& n, int& dim, std::vector< std::vector<int> >& Z,
    ...
    while ((line = readline(fp)) != NULL) {
        if (args.useMyMips == 2) Zinv.push_back(std::vector<int>());
        ...
        while(1) {
            ...
            Z[j - 1].push_back(n);
            if (args.useMyMips == 2) Zinv[n].push_back(j-1);
        }
        n++;
        free(line);
    }
