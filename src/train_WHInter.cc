...
int main(int argc, char **argv) {
    ...
    file_preproc.open(args.pathResults + 
                      ...
                      "_M" + to_string(args.useMyMips) + "_Bo" + to_string(args.typeBound) + "_F" + to_string(args.F) + 
    ...
    //
    // ofstream file_phi;
    // file_phi.open("/Users/lemorvan/Desktop/mutimpact/projects/Lasso_interactions/results/WHInter_eps_MIPS/" +
    //               file + "_nlam" + to_string(args.nlambda) + "_rat" +  str_rat + 
    //               "_max" + to_string(args.maxSelectedFeatures) + "_Bi" + to_string(args.useBias) +
    //               "_M" + to_string(args.useMyMips) + "_Bo" + to_string(args.typeBound) + "_F" + to_string(args.F) + "_phi.csv");
    // ofstream file_submodel_size;
    // file_submodel_size.open("/Users/lemorvan/Desktop/mutimpact/projects/Lasso_interactions/results/WHInter_eps_MIPS/" + 
    //                         file + "_nlam" + to_string(args.nlambda) + "_rat" +  str_rat + 
    //                         "_max" + to_string(args.maxSelectedFeatures) + "_Bi" + to_string(args.useBias) +
    //                         "_M" + to_string(args.useMyMips) + "_Bo" + to_string(args.typeBound) + "_F" + to_string(args.F) + "_cd.csv");
    ofstream file_model;
    ...
    for (int t = 0; t < args.nlambda; t++) {
        ...
        for (int s = 0; s < m; s++) {
            ...
            if (w[j] != 0) {
                ...
                // Check wether mips.best_ip should be updated with the features that were just removed from the model
                int ind_0 = key[j][0];
                int ind_1 = key[j][1];
                // cout << "Pre-solve: feature with 0 coef in model: " << ind_0 << " " << ind_1 << endl;
        ...
        for (int iterB = 1; iterB < 10000000; iterB++) {
            ...
            for (int idx_b = 0; idx_b < dim; ++idx_b){
                vector<double> ref_res = ref[ref_id[idx_b]];
                bb->compute_coef_and_compute_bound(phi, ref_res, Z[idx_b], mips->get_best_ip()[idx_b]);
                if (bb->get_bound() > lam){
            ...
            for (int iter = 1; iter <= 1000000; iter++) {
                ...
                if (iter % args.F == 1){
                    ...
                    cout << "P_new: " << P_subproblem << " loss:" << loss << " L1norm:" << L1norm << endl;
